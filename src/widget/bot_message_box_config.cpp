...
bool MessageBoxConfig::init()
{
    const App& app = App::getInstance();
    const char* fileName = app.getConfig().getMessageBoxConfigFile().c_str();
    ...
    const GameLib& lib = app.getGameLib();

    m_boxRect = lib.getRect(boxRectName);
    if (!m_boxRect)
    {
    ...
    m_buttonRect = lib.getRect(buttonRectName);
    if (!m_buttonRect)
    {
    ...
    m_boxFillColor = lib.getColor(boxFillColorName);
    if (!m_boxFillColor)
    {
    ...
    m_textColor = lib.getColor(textColorName);
    if (!m_textColor)
    {
    ...
    m_boxBorderColor = lib.getColor(boxBorderColorName);
    if (!m_boxBorderColor)
    {
    ...
    m_boxPos[0] = app.getViewportWidth() / 2.0f;
    m_boxPos[1] = app.getViewportHeight() / 2.0f;
