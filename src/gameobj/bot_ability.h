...
public:
    ...
    const Clock::time_point& getLastFireTime() const
    {
    ...
    {
        return static_cast<const FireAbilityTemplate*>(m_template)->getBulletTemplate();
    }

    void rotateFirePos(float referenceX, float referenceY, float directionX, float directionY);

    bool shouldFire(const Clock::time_point& t) const;
